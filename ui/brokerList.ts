...
class BrokerList {
  ...
  constructor(visibility: Visibility) {
    ...
    this.infoDiv_ = new InfoDiv(['Name: ', 'Endpoint: ', 'Owner: ']);
  ...
  getSelected(): BrokerRegistration | null {
  ...
  setVisibility(visibility: Visibility) {
    this.infoDiv_.setVisibility([visibility.name, visibility.endpoint, visibility.owner]);
    applyVisibility(this.RDFContainer_, visibility.RDF);
  }

  onNew(key: string, sensor: BrokerRegistration) {
    const adding: Option = {
      tx: sensor,
  ...
  onChange(key: string, broker: BrokerRegistration) {
    if (this.infoOptions_.has(key)) {
      const child = this.infoOptions_.get(key);
      child.tx = broker;
  ...
  private setInfo(broker: BrokerRegistration) {
    this.info_.style.display = "block";
    this.infoDiv_.setValue([broker.metadata.name, broker.metadata.endpoint, broker.input]);
...
export { BrokerList, type Visibility };